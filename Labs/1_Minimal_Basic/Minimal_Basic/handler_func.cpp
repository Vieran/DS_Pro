...
void MainWindow::let_handler(Statement *sta) {
    ...
    QString purify_sta = let_pattern.cap(2);
    try {
        construct_exp_tree(sta, purify_sta);
    } catch (QString error_msg) {
        error_occur = true;
        add_err_line(let_pattern.cap(1).toUInt());
...
void MainWindow::print_handler(Statement *sta) {
    ...
    QString purify_sta = print_pattern.cap(2);
    try {
        construct_exp_tree(sta, purify_sta);
    } catch (QString error_msg) {
        error_occur = true;
        add_err_line(print_pattern.cap(1).toUInt());
        error_handler(error_msg);
    }
}
...
void MainWindow::ifthen_handler(Statement *sta) {
    ...
    QString purify_sta = if_pattern.cap(2);
    try {
        construct_exp_tree_ifthen(sta, purify_sta);
    } catch (QString error_msg) {
        error_occur = true;
        add_err_line(if_pattern.cap(1).toUInt());
        error_handler(error_msg);
    }
}